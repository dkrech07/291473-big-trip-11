...
export default class PointController {
  ...
  render(point, mode) {
    ...
    const newFormClickHandler = (evt) => {
      ...
      const data = this._formComponent.getData(point); // Точка не создается если заменить point на this.point
      this._onDataChange(this, point, data); // Точка не создается если заменить point на this.point
      this._newPointButton.removeAttribute(`disabled`);
